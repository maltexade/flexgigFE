...
// === WebAuthn: safe, responsive prefetch helper (v2 -> v2.patched) ===
// Goal: eliminate delayed biometric prompt by making auth options ready ASAP without breaking UX.
// Strategy:
//  - Attempt immediate prefetch repeatedly for a short window while app initializes.
//  - Trigger prefetch on first user gesture (pointerdown) to guarantee user-gesture context for any follow-up call.
//  - Wrap showReauthModal to ensure prefetch has been attempted right before showing modal.
(function(){
  try {
    if (window.__webauthn_quick_prefetch_installed) return;
    window.__webauthn_quick_prefetch_installed = true;

    const MAX_ATTEMPTS = 6;
    const ATTEMPT_INTERVAL = 200; // ms
    let attempts = 0;
    let done = false;
    ...
    async function tryOnce(){
      try {
        if (typeof window.getAuthOptionsWithCache === 'function') {
          ...
          console.debug('[webauthn-prefetch] getAuthOptionsWithCache succeeded');
          return true;
        } else if (typeof window.prefetchAuthOptions === 'function') {
          await window.prefetchAuthOptions().catch(()=>{});
          done = true;
          window.__webauthn_prefetched_at = Date.now();
          console.debug('[webauthn-prefetch] prefetchAuthOptions succeeded');
          return true;
        }
      } catch(e){
        console.warn('[webauthn-prefetch] tryOnce error', e);
      }
      return false;
    }

    // Attempt immediate tries (non-blocking)
    (function bootAttempts(){
      tryOnce().then(ok=>{
        if (ok) return;
        const id = setInterval(async ()=>{
          attempts++;
          const ok2 = await tryOnce();
          if (ok2 || attempts >= MAX_ATTEMPTS) clearInterval(id);
        }, ATTEMPT_INTERVAL);
      });
    })();

    // On first meaningful user gesture, try again (guarantees gesture context)
    function onFirstGesture(){
      try {
        if (done) { removeGestureListeners(); return; }
        tryOnce().catch(()=>{});
        removeGestureListeners();
      } catch(e){ removeGestureListeners(); }
    }
    ...
    // Wrap showReauthModal so we ensure one last prefetch attempt (and wait up to a short timeout)
    if (typeof window.showReauthModal === 'function' && !window.__webauthn_showReauthWrapped) {
      ...
      window.showReauthModal = async function(opts){
        try {
          // If not yet done, attempt once and wait briefly (max 350ms) for it to finish
          if (!done) {
            const p = tryOnce();
            const timeout = new Promise(resolve => setTimeout(resolve, 350));
            await Promise.race([p, timeout]);
          }
        } catch(e){ console.warn('[webauthn-prefetch] pre-show attempt failed', e); }
        return orig.apply(this, arguments);
      };
    }

    console.debug('[webauthn-prefetch] helper installed');
  } catch(e){
    console.warn('[webauthn-prefetch] init error', e);
  }
})(); // end helper
