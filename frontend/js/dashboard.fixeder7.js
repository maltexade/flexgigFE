console.log('[BOOT] dashboard.fixed6.js loaded at', new Date().toISOString());
window.addEventListener('visibilitychange', () => { console.log('[VISIBILITY] document.visibilityState ->', document.visibilityState, new Date().toISOString()); });
...
/* INSERT: Visual fill function for the 4 PIN boxes */
function fillReauthInputs(maskChar) {
  maskChar = maskChar || '•';
  try {
    var inputs = (typeof getReauthInputs === 'function') ? getReauthInputs() : (document.querySelectorAll && Array.from(document.querySelectorAll('.reauth-pin-input')) || []);
    if (!inputs || !inputs.length) return;
    inputs.forEach(function(inp){
      try {
        inp.value = maskChar;
        inp.classList && inp.classList.add && inp.classList.add('filled');
        inp.blur && inp.blur();
      } catch (e) {
        console.warn('fillReauthInputs: input update error', e);
      }
    });
    window.processing = true;
    console.log('[PIN] Inputs visually filled with mask');
  } catch (err) {
    console.error('fillReauthInputs error', err);
  }
}
...
bioBtn.addEventListener('click', async () => {
  ...
  console.log('[reauth] Biometric verification SUCCESS (enhanced flow)', verifyData);

  try { fillReauthInputs('•'); } catch (e) { console.warn('fillReauthInputs failed', e); }

  try {
    localStorage.removeItem('reauthPending');
    localStorage.setItem('reauthCompletedAt', Date.now().toString());
  } catch (e) { console.warn('Could not update reauth localStorage flags', e); }
  ...
  try {
    withLoader(function(){
      return (async function(){
        try {
          ...
          if (typeof onSuccessfulReauth === 'function') {
            console.log('[reauth] Calling onSuccessfulReauth now');
            await onSuccessfulReauth(verifyData);
          } else if (typeof getSession === 'function') {
            console.log('[reauth] getSession fallback');
            await (typeof safeCall === 'function' ? safeCall(getSession) : getSession());
          } else {
            console.log('[reauth] No success handler found; continuing');
          }

          console.log('[reauth] Biometric post-success processing completed');
        } catch(innerErr) {
          console.error('[reauth] Error during onSuccessfulReauth processing', innerErr);
        } finally {
    ...
    console.error('[reauth] withLoader error after biometric success', e);
    window.processing = false;
  }
  return;
} else {
...
/* INSERT: Cross-tab reauth persistence and UI lock helpers */
(function installReauthCrossTabLock(){
  if (window.__reauthCrossTabInstalled) return;
  window.__reauthCrossTabInstalled = true;

  function showReauthLockOverlay() {
    if (document.getElementById('reauthLockOverlay')) return;
    const overlay = document.createElement('div');
    overlay.id = 'reauthLockOverlay';
    ...
    overlay.innerHTML = `<div style="padding:20px;border-radius:8px;background:#fff;color:#111;max-width:420px;text-align:center;">
      <strong>Action required</strong>
      <div style="margin-top:8px;font-size:13px;color:#444;">Please re-authenticate to continue using the dashboard.</div>
    </div>`;
    document.body.appendChild(overlay);
    console.log('[LOCK] reauth lock overlay shown');
  }

  function hideReauthLockOverlay() {
    const el = document.getElementById('reauthLockOverlay');
    if (el) el.remove();
    console.log('[LOCK] reauth lock overlay removed');
  }

  window.addEventListener('storage', (e) => {
    try {
      if (e.key === 'reauthPending') {
        console.log('[STORAGE] reauthPending changed', { old: e.oldValue, new: e.newValue });
        if (e.newValue) {
          try { initReauthModal && initReauthModal({ show: true }); } catch(err){ console.error('initReauthModal(storage) error', err); }
          showReauthLockOverlay();
        } else {
          try { hideReauthModal && hideReauthModal(); } catch(err){ /*noop*/ }
          hideReauthLockOverlay();
        }
      }
  ...
  try {
    if (localStorage.getItem('reauthPending')) {
      console.log('[STARTUP] reauthPending present on load - showing modal & overlay');
      try { initReauthModal && initReauthModal({ show: true }); } catch(e){ console.error('initReauthModal(startup) error', e); }
      showReauthLockOverlay();
    }
  } catch (err) {
    console.warn('Could not read reauthPending at startup', err);
  }

  window.showReauthLockOverlay = showReauthLockOverlay;
  window.hideReauthLockOverlay = hideReauthLockOverlay;
})();
          window.location.href = '/reset-pin.html';
        });
        link.__bound = true;
...
/* INSERT: Delegated capture-level biometric click handler
   (installed by automation) */
(function installDelegatedBioClick(){
  if (window.__delegatedBioClickInstalled) return;
  console.log('[DELEGATE] Installing delegated biometric click catcher');

  const selector = '#pinBiometricBtn';
  ['pointerdown','touchstart','click'].forEach(evtName => {
    document.addEventListener(evtName, function delegatedBioHandler(e){
      try {
        const target = e.target && (e.target.matches && e.target.matches(selector) ? e.target : (e.target.closest ? e.target.closest(selector) : null));
        if (!target) return;
        console.log('[DELEGATE] captured biometric gesture', { event: evtName, time: new Date().toISOString(), visible: document.visibilityState });

        e.preventDefault && e.preventDefault();
        e.stopImmediatePropagation && e.stopImmediatePropagation();

        try { __immediateBiometricClickHandler && __immediateBiometricClickHandler(e); }
        catch (err) { console.error('[DELEGATE] __immediateBiometricClickHandler threw', err); }

      } catch (err) {
        console.error('[DELEGATE] delegatedBioHandler error', err);
      }
    }, { capture: true, passive: false });
  });

  window.__delegatedBioClickInstalled = true;
})();
