/* resetPin.js – v7 (merged + hardening)
   - All original features preserved from your v7:
     * Resend counter shows clean "Resend OTP" after countdown
     * Multi / single OTP input support with auto-submit
     * Open email client helper
     * Full notifier fallback (toasts, Web Notification)
     * Wire/unwire idempotent handlers, debug helpers
   - Hardened with:
     * clearReauthFlags() to avoid re-opened modals
     * gracefulCloseAll() which prefers ModalManager.closeAll() then DOM fallback
     * Safe one-time pin-status-changed listener that closes all modals
     * Defensive postJson/getUserEmail and more robust event cleanup
*/
(function rpWireResetFlow_v7(){
  ...
  // ---- Notification helper (original, extended) ----
  ...
  async function getUserEmail() {
    ...
    try {
      ...
      for (const fn of candidates) {
        if (typeof fn === 'function') {
          try {
            ...
            if (!session) continue;
            if (session.email) { log('getUserEmail: found', session.email); return session.email; }
            if (session.user && session.user.email) { log('getUserEmail: found', session.user.email); return session.user.email; }
            if (session.data && session.data.user && session.data.user.email) { log('getUserEmail: found', session.data.user.email); return session.data.user.email; }
          } catch (e) {
      ...
      const lsEmail = localStorage.getItem('userEmail') || localStorage.getItem('email') || '';
      if (lsEmail) { log('getUserEmail: found in localStorage', lsEmail); return lsEmail; }
  ...
  // ---- Clear canonical reauth flags to avoid modals auto re-opening ----
  function clearReauthFlags() {
    try {
      if (window.fgReauth && typeof window.fgReauth.completeReauth === 'function') {
        try { window.fgReauth.completeReauth().catch(()=>{}); } catch(e){}
      }
    } catch(e){}
    try { localStorage.removeItem('fg_reauth_required_v1'); } catch(e){}
    try { localStorage.removeItem('reauthPending'); } catch(e){}
    try { sessionStorage.removeItem('fg_reauth_required_v1'); } catch(e){}
    dbg('clearReauthFlags: cleared known flags');
  }

  // ---- Graceful close all modals: prefer ModalManager.closeAll(), otherwise DOM fallback ----
  function gracefulCloseAll() {
    try {
      if (window.ModalManager && typeof window.ModalManager.closeAll === 'function') {
        window.ModalManager.closeAll();
        dbg('gracefulCloseAll: used ModalManager.closeAll');
        return;
      }
    } catch(e){ dbg('gracefulCloseAll: ModalManager.closeAll threw', e); }

    try {
      // Known ids - extend as needed
      const known = ['pinModal','resetPinModal','securityPinModal','securityModal','settingsModal','updateProfileModal'];
      known.forEach(id => {
        const el = document.getElementById(id);
        if (el) {
          try { el.classList.add('hidden'); el.style.display='none'; el.setAttribute('aria-hidden','true'); el.setAttribute('inert',''); } catch(e){}
        }
      });
      // remove backdrops
      document.querySelectorAll('.modal-backdrop').forEach(b => b.remove());
      dbg('gracefulCloseAll: used DOM fallback');
    } catch(e){ dbg('gracefulCloseAll fallback error', e); }
  }
  ...
  // ---- OTP verify handler (with graceful closeAll) ----
  async function verifyOtpSubmit(e) {
    ...
    try {
      ...
      if (status >= 200 && status < 300) {
        ...
        // Setup one-time listener for final pin setup success – close ALL modals smoothly
        const onPinStatusChanged = (ev) => {
          log('onPinStatusChanged: PIN setup complete event received', ev && ev.detail);
          try {
            // Clear reauth flags to prevent re-show
            clearReauthFlags();

            // Use ModalManager.closeAll() for smooth modal closure
            if (window.ModalManager && typeof window.ModalManager.closeAll === 'function') {
              try {
              ...
              } catch(e) { dbg('ModalManager.closeAll threw', e); gracefulCloseAll(); }
            } else {
              // Fallback: Close known modals manually
              dbg('onPinStatusChanged: ModalManager.closeAll not available - using DOM fallback');
              gracefulCloseAll();
            }
          } catch (e) {
            err('onPinStatusChanged: error during closeAll', e);
          } finally {
            // Success notification AFTER modals are closed
            notify({
              type: 'success',
            ...
            // Emit a guarded local event so other modules can act without forcing a session sync
            try { window.dispatchEvent(new CustomEvent('pin-reset:complete', { detail: { source: 'resetPin.js', at: Date.now() } })); } catch(e){ dbg('dispatch pin-reset:complete failed', e); }
        ...
        try { document.removeEventListener('pin-status-changed', window.__rp_handlers._pinStatusHandler); } catch(_) {}
        ...
        const errMsg = body?.error?.message || body?.message || 'OTP verification failed';
  ...
  async function _onTriggerClicked() {
    log('onTriggerClicked: start');
    const btn = $(TRIGGER_ID);
  ...
  function wire() {
    ...
    (async () => {
      ...
      } catch (e) { dbg('wire display email error', e); }
    })();
  ...
  window.__rp_wire_debug = Object.assign(window.__rp_wire_debug || {}, {
    ...
    wire, rewire: wire, verifyOtpSubmit, resendOtpHandler, startResendCountdown, notify, gracefulCloseAll, clearReauthFlags
  });
