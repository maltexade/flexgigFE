...
/* =========================
   Robust Reauth & Biometric UX Patch
   Appended by assistant to create dashboard.fixed7.js
   - Ensures first-click works (prefetch on modal open & pointerdown/touchstart)
   - Persists reauth requirement across tabs (localStorage)
   - Shows withLoader during verify and fills 4 PIN boxes visually
   - Uses short cache freshness (5s) to avoid challenge mismatch
   ========================= */

(function(){
  // -- Helpers (do not overwrite if present) --
  ...
  // -- Reauth persistence across tabs --
  window.requireReauth = function() {
    try {
      localStorage.setItem('reauth_pending', '1');
      window.__reauth_pending = true;
      try { if (typeof openPinModal === 'function') openPinModal(); } catch(e){}
      try { if (typeof showBiometricPanel === 'function') showBiometricPanel(); } catch(e){}
      try { window.prefetchAuthOptions && window.prefetchAuthOptions(); } catch(e){}
    } catch(e) { console.warn('requireReauth err', e); }
  };

  window.clearReauthRequirement = function() {
    try {
      localStorage.removeItem('reauth_pending');
      window.__reauth_pending = false;
      try { if (typeof hideReauthModal === 'function') hideReauthModal(); } catch(e){}
    } catch(e) { console.warn('clearReauthRequirement err', e); }
  };

  window.addEventListener('storage', function(ev){
    try {
      if (!ev) return;
      if (ev.key === 'reauth_pending') {
        if (ev.newValue) {
          try { if (typeof openPinModal === 'function') openPinModal(); } catch(e){}
          try { window.prefetchAuthOptions && window.prefetchAuthOptions(); } catch(e){}
        } else {
  ...
  // enforce on load
  try {
    if (localStorage.getItem('reauth_pending')) {
      window.__reauth_pending = true;
      setTimeout(function(){
        try { if (typeof openPinModal === 'function') openPinModal(); } catch(e){}
        try { window.prefetchAuthOptions && window.prefetchAuthOptions(); } catch(e){}
  ...
  // -- Fill PIN boxes helper --
  window.fillPinDigitsFromAssertion = function(signatureArrayBuffer) {
  try {
    // Normalize to Uint8Array if possible
    var buf = null;
    if (!signatureArrayBuffer) {
      buf = null;
    } else if (typeof signatureArrayBuffer === 'string') {
      // assume base64url
      try {
        var b64 = signatureArrayBuffer.replace(/-/g, '+').replace(/_/g, '/');
    ...
    } else if (signatureArrayBuffer instanceof Uint8Array) {
      buf = signatureArrayBuffer;
    } else if (signatureArrayBuffer instanceof ArrayBuffer) {
      buf = new Uint8Array(signatureArrayBuffer);
    } else if (signatureArrayBuffer && signatureArrayBuffer.buffer instanceof ArrayBuffer) {
      buf = new Uint8Array(signatureArrayBuffer.buffer, signatureArrayBuffer.byteOffset || 0, signatureArrayBuffer.byteLength || signatureArrayBuffer.buffer.byteLength);
    } else {
      buf = null;
    }
    ...
    if (buf && buf.length >= 1) {
      digits = [];
      for (var i = 0; i < 4; i++) {
        digits.push(String((buf[i] || 0) % 10));
      }
    }

    var selectors = [
      '#pinModal input[type=\"text\"]',
      '#reauthModal input[type=\"text\"]',
      '.pin-box input',
      '.pin-input',
      '.code-input input',
      'input.pin-digit',
    ...
    for (var si = 0; si < selectors.length; si++) {
      try {
        ...
        if (nodes && nodes.length >= 4) {
          for (var j = 0; j < 4; j++) {
            try { nodes[j].value = digits[j]; nodes[j].dispatchEvent(new Event('input', {bubbles: true})); } catch(e){}
          }
          return true;
        }
      } catch(e){}
    ...
    // fallback: first four text inputs
    var allInputs = document.querySelectorAll('input[type=\"text\"]');
    if (allInputs && allInputs.length >= 4) {
      for (var k = 0; k < 4; k++) {
        try { allInputs[k].value = digits[k]; allInputs[k].dispatchEvent(new Event('input', {bubbles: true})); } catch(e){}
      }
      return true;
    }
  } catch (e) {
    console.warn('fillPinDigitsFromAssertion error', e);
  }
  return false;
};

  // -- Prefetch auth options (override/define) --
  window.prefetchAuthOptions = window.prefetchAuthOptions || (async function prefetchAuthOptions() {
    // Debounced/deduped prefetch that avoids calling server when userId is unknown.
    try {
      if (window.__prefetchPromise) {
        // reuse existing inflight promise
        return window.__prefetchPromise;
      }
      ...
      var p = (async function(){
        try {
          ...
          if (!storedId) {
            console.log('[prefetchAuthOptions] no stored credential, skipping prefetch');
            return null;
          }

          // Try to get a cached userId quickly. If none, attempt getSession but only wait briefly.
          var uid = window.__webauthn_userId || null;
          if (!uid && typeof getSession === 'function') {
            try {
              // wait for at most 300ms for getSession to return
              var sessPromise = safeCall(getSession);
              uid = await Promise.race([
                sessPromise,
                new Promise(function(resolve){ setTimeout(function(){ resolve(null); }, 300); })
              ]);
              if (uid && uid.user) uid = uid.user.uid || uid.user.id || null;
              if (uid) window.__webauthn_userId = uid;
            } catch(e){
              uid = window.__webauthn_userId || null;
            }
          }

          if (!uid) {
            // Server requires userId for options. Avoid calling server without it.
            console.log('[prefetchAuthOptions] userId unavailable; skipping prefetch to avoid 400');
            return null;
          }

          var res = await fetch((window.__SEC_API_BASE || API_BASE) + '/webauthn/auth/options', {
            method: 'POST',
            credentials: 'include',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ credentialId: storedId, userId: uid })
          });

          if (!res.ok) {
            var txt = null;
            try { txt = await res.text(); } catch(e){ txt = String(res.status); }
            console.warn('[prefetchAuthOptions] options fetch not ok', txt);
            return null;
          }

          var publicKey = await res.json();
          try {
            if (publicKey.challenge) publicKey.challenge = window.fromBase64Url ? window.fromBase64Url(publicKey.challenge) : fromBase64Url(publicKey.challenge);
            if (Array.isArray(publicKey.allowCredentials)) {
              publicKey.allowCredentials = publicKey.allowCredentials.map(function(c){
                return {
                  type: c.type || 'public-key',
                  transports: c.transports || ['internal'],
                  id: (window.fromBase64Url ? window.fromBase64Url(c.id) : fromBase64Url(c.id))
                };
              });
            }
          } catch(e){ console.warn('[prefetchAuthOptions] conversion error', e); }
          ...
          window.__prefetchPromise = null;
        }
      })();

      window.__prefetchPromise = p;
      return p;
    } catch(err) {
      window.__prefetchInFlight = false;
      window.__prefetchPromise = null;
      ...
      if (!res.ok) {
        try { console.warn('[prefetchAuthOptions] options fetch not ok', await res.text()); } catch(e){}
        window.__prefetchInFlight = false;
        return;
      }
  ...
  // ensure triggers prefetch on pointerdown/touchstart/mouseenter
  function addPrefetchBindingsTo(el) {
    try {
      if (!el) return;
      el.addEventListener('pointerdown', function(){ try{ window.prefetchAuthOptions(); }catch(e){} }, {passive:true});
      el.addEventListener('touchstart', function(){ try{ window.prefetchAuthOptions(); }catch(e){} }, {passive:true});
      el.addEventListener('mouseenter', function(){ try{ window.prefetchAuthOptions(); }catch(e){} }, {passive:true});
    } catch(e){}
  }

  // attach to common selectors and modal inner buttons
  var selectors = ['#bioBtn', '#pinBiometricBtn', '.biometric-button', '[data-bio-button]'];
  for (var i=0;i<selectors.length;i++) {
    try { var el = document.querySelector(selectors[i]); if (el) addPrefetchBindingsTo(el); } catch(e){}
  }

  // modal inner buttons
  var modalInnerSelectors = ['#pinBiometricBtn', '#modalBioBtn', '.modal .biometric-button', '[data-modal-bio]'];
  for (var j=0;j<modalInnerSelectors.length;j++) {
    try { var el2 = document.querySelector(modalInnerSelectors[j]); if (el2) addPrefetchBindingsTo(el2); } catch(e){}
  }

  // MutationObserver fallback: if modal appears later, ensure prefetch binding
  try {
        ...
        for (var m=0;m<muts.length;m++) {
          ...
          for (var a=0;a<added.length;a++) {
            ...
            for (var k=0;k<selectors.length;k++) {
              try {
                var maybe = node.querySelector(selectors[k]);
                if (maybe) addPrefetchBindingsTo(maybe);
              } catch(e){}
            }
            for (var k2=0;k2<modalInnerSelectors.length;k2++) {
              try {
                var maybe2 = node.querySelector(modalInnerSelectors[k2]);
                if (maybe2) addPrefetchBindingsTo(maybe2);
              } catch(e){}
            }
          }
        }
  ...
  // -- Robust click handler: attach to primary trigger as well (non-destructive override) --
  try {
    var bioTrigger = document.getElementById('bioBtn') || document.getElementById('pinBiometricBtn') || document.querySelector('.biometric-button') || document.querySelector('[data-bio-button]');
    if (bioTrigger) {
      // remove previous click to reduce duplicates (best-effort)
      try { bioTrigger.removeEventListener('click', window.__assistant_bio_click_handler); } catch(e){}
      window.__assistant_bio_click_handler = async function(e){
        console.log('[reauth] Biometric button clicked (PIN modal)');
        try {
          // verify local enable flag or stored credential
          var _flag = (localStorage.getItem('biometricForLogin') || localStorage.getItem('__sec_bioLogin') || localStorage.getItem('security_bio_login') || '').toLowerCase();
          var storedCred = localStorage.getItem('credentialId') || localStorage.getItem('webauthn-cred-id') || localStorage.getItem('webauthn_cred') || '';
          var localEnabled = !!storedCred || ['true','1','yes'].indexOf(_flag) !== -1;
          if (!localEnabled) {
            console.warn('[reauth] Biometric login not enabled locally.');
            try { safeCall(notify, 'Biometric login not enabled', 'warn'); } catch(e){}
            return;
          }
          ...
          var storedId = storedCred || null;
          var uid = window.__webauthn_userId || null;
          if (!uid && typeof getSession === 'function') {
            try { var sess = await safeCall(getSession); uid = sess && sess.user && (sess.user.uid || sess.user.id); if (uid) window.__webauthn_userId = uid; } catch(e){}
          }

          // decide cached usage
          var publicKey = window.__cachedAuthOptions || null;
          var freshMs = 5 * 1000;
          if (!(publicKey && window.__cachedAuthOptionsFetchedAt && ((Date.now() - window.__cachedAuthOptionsFetchedAt) <= freshMs))) {
            // stale or missing
            try {
              ...
              console.error('[reauth] Auth options fetch failed (click)', fetchErr);
              try { safeCall(notify, 'Could not start biometric auth', 'error'); } catch(e){}
              return;
            }
          } else {
            console.log('[reauth] using cached auth options for immediate prompt');
          }

          // now call navigator.credentials.get in gesture
          var assertion;
          try {
            assertion = await navigator.credentials.get({ publicKey: publicKey });
          } catch(navErr) {
            console.error('[reauth] navigator.credentials.get error', navErr);
            try { safeCall(notify, 'Biometric prompt failed', 'error'); } catch(e){}
            return;
          }
          if (!assertion) {
            console.warn('[reauth] No assertion returned (maybe cancelled)');
            return;
          }

          // visually fill pin boxes
          try { window.fillPinDigitsFromAssertion(assertion.response && assertion.response.signature ? assertion.response.signature : null); } catch(e){}

          // run server verify inside withLoader to block input
          try {
            var verifyResponse = await withLoader(async function(){
              var payload = {
                id: assertion.id,
                rawId: (window.toBase64Url ? window.toBase64Url(assertion.rawId) : (function(b){var s='';for(var i=0;i<new Uint8Array(b).length;i++) s+=String.fromCharCode(new Uint8Array(b)[i]);return btoa(s).replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'');})(assertion.rawId)),
                type: assertion.type,
                response: {
                  authenticatorData: (window.toBase64Url ? window.toBase64Url(assertion.response.authenticatorData) : (function(b){var s='';for(var i=0;i<new Uint8Array(b).length;i++) s+=String.fromCharCode(new Uint8Array(b)[i]);return btoa(s).replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'');})(assertion.response.authenticatorData)),
                  clientDataJSON: (window.toBase64Url ? window.toBase64Url(assertion.response.clientDataJSON) : (function(b){var s='';for(var i=0;i<new Uint8Array(b).length;i++) s+=String.fromCharCode(new Uint8Array(b)[i]);return btoa(s).replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'');})(assertion.response.clientDataJSON)),
                  signature: (window.toBase64Url ? window.toBase64Url(assertion.response.signature) : (function(b){var s='';for(var i=0;i<new Uint8Array(b).length;i++) s+=String.fromCharCode(new Uint8Array(b)[i]);return btoa(s).replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'');})(assertion.response.signature)),
                  userHandle: assertion.response.userHandle ? (window.toBase64Url ? window.toBase64Url(assertion.response.userHandle) : null) : null
                }
              };
              var sess = await safeCall(getSession);
              var uidLocal = sess && (sess.user && (sess.user.uid || sess.user.id));
              var r = await fetch((window.__SEC_API_BASE || API_BASE) + '/webauthn/auth/verify', {
                method: 'POST',
                credentials: 'include',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(Object.assign({}, payload, { userId: uidLocal }))
              });
              return r;
            });
          } catch(errVerify) {
            console.error('[reauth] Biometric verification error (withLoader)', errVerify);
            try { safeCall(notify, 'Biometric verification failed', 'error'); } catch(e){}
            return;
          }

          // parse result
          var verifyJson = null;
          try { verifyJson = await verifyResponse.json(); } catch(e){ verifyJson = { error: 'Invalid JSON' }; }
          if (!verifyResponse.ok) {
            console.warn('[reauth] Biometric failed server side', verifyJson);
            try { safeCall(notify, 'Biometric verification failed', 'error'); } catch(e){}
            return;
          }
          if (verifyJson.verified) {
            console.log('[reauth] Biometric verification success', verifyJson);
            try { window.clearReauthRequirement(); } catch(e){}
            try { if (typeof hideReauthModal === 'function') hideReauthModal(); } catch(e){}
            try { if (typeof onSuccessfulReauth === 'function') onSuccessfulReauth(); } catch(e){}
          } else {
            console.warn('[reauth] Biometric verification failed or cancelled', verifyJson);
            try { safeCall(notify, 'Biometric verification failed', 'error'); } catch(e){}
          }

        } catch(finalErr) {
          console.error('[reauth] Biometric click handler error', finalErr);
        }
      };
      bioTrigger.addEventListener('click', window.__assistant_bio_click_handler);
    }
  } catch(e){ console.warn('attach bioTrigger handler failed', e); }

  // -- Ensure disableBiometrics is non-blocking (wrap existing if necessary) --
  try {
    if (typeof disableBiometrics === 'function') {
      var _origDisable = disableBiometrics;
      window.disableBiometrics = function(){
        try { _origDisable(); } catch(e){}
