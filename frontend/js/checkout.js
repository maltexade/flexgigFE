    ...
    try {
        ...
        if (!selectedProvider) {
            ...
            if (slider) {
                ...
                const providerClass = classes.find(c =>
        ...
        let provider = ['mtn', 'airtel', 'glo', 'ninemobile'].find(p =>
            selectedProvider.classList.contains(p)
        );
        ...
        if (!selectedPlan) {
            ...
            if (selectedBox && selectedBox.dataset.planId) {
                selectedPlan = {
                    ...
                    dataAmount:
                        selectedBox.dataset.dataAmount ||
                        selectedBox.querySelectorAll('div')[1]?.textContent?.trim() ||
                        'N/A',
                    validity:
                        selectedBox.dataset.validity ||
                        selectedBox.querySelectorAll('div')[2]?.textContent?.trim() ||
                        'N/A',
...
function isProfileComplete() {
    ...
    const checkoutPhone =
        document.getElementById('phone-input')?.value?.trim() || state.number; // fallback if already saved

    return !!(checkoutPhone && checkoutPhone.length >= 10);
}
...
function openCheckoutModal(data) {
    ...
    console.log('[PRICE DEBUG] Saved to localStorage:', {
        ...
        isSpecial:
            checkoutInfo.planId.includes('special') ||
    ...
    try {
        ...
        if (priceEl) {
            priceEl.textContent = `₦${checkoutInfo.price.toLocaleString('en-NG', {
        ...
        if (providerEl) {
            let providerKey =
                checkoutInfo.provider.toLowerCase() === '9mobile'
                    ? 'ninemobile'
        ...
        if (dataEl)
            dataEl.textContent = `${checkoutInfo.dataAmount} / ${checkoutInfo.validity}`;

        const amountEls = modal.querySelectorAll('.info-row:last-child .value');
        amountEls.forEach(el => {
            el.textContent = `₦${checkoutInfo.price.toLocaleString('en-NG', {
                minimumFractionDigits: 2
            })}`;
        });
        ...
        if (window.ModalManager && typeof window.ModalManager.openModal === 'function') {
            ...
            if (ModalManager.getOpenModals().includes('checkoutModal')) {
                console.warn(
                    '[checkout] checkoutModal was already in stack — forcing clean close first'
                );
...
async function continueCheckoutFlow() {
    ...
    try {
        ...
        // ✅ FIX: Check initial status before polling
...
async function processPayment() {
    ...
    if (!response.ok) {
        if (result.error === 'insufficient_balance') {
            throw new Error(
                `Insufficient balance: ₦${result.current_balance?.toLocaleString() || '0'}`
            );
        }
...
async function updateReceiptToSuccess(result) {
    ...
    const providerKey =
        data.provider?.toLowerCase() === '9mobile'
            ? 'ninemobile'
            : data.provider?.toLowerCase() || '';
    const svg = window.svgShapes?.[providerKey] || '';

    document.getElementById('receipt-provider').innerHTML = `${svg} ${
        data.provider?.toUpperCase() || 'Unknown'
    }`;
    document.getElementById('receipt-phone').textContent = data.number || '—';
    document.getElementById('receipt-plan').textContent = data.dataAmount
        ? `${data.dataAmount} / ${data.validity || '—'}`
        : 'Data Bundle';
    ...
    document.getElementById('receipt-balance').textContent = `₦${Number(
        result?.new_balance ?? data?.new_balance ?? 0
    ).toLocaleString('en-NG')}`;
    document.getElementById('receipt-time').textContent = new Date().toLocaleString('en-NG', {
        dateStyle: 'medium',
        timeStyle: 'short'
    });
    ...
    if (typeof window.renderRecentTransactions === 'function') {
        try {
            ...
            try {
                ...
                console.log(
                    `[checkout] Loaded ${currentRecent.length} existing recent txs from localStorage`
                );
            } catch (e) {
            ...
            currentRecent = currentRecent.filter(
                tx =>
            ...
            console.log(
                '[checkout] Updated list (after dedupe & slice):',
                currentRecent.length,
                'items'
            );
            console.table(
                currentRecent.map(tx => ({
                    phone: tx.phone,
        ...
        console.warn(
            "[checkout] renderRecentTransactions function not found — list won't update"
        );
    }
}
...
function updateReceiptToFailed(errorMessage) {
    ...
    icon.innerHTML = `
        <svg width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <path d="M24 24L40 40M40 24L24 40" stroke="#EF4444" stroke-width="4" stroke-linecap="round"/>
        </svg>
    `;
    ...
    actions.innerHTML = `
        <button class="receipt-btn" id="receipt-done">Close</button>
    `;
...
function updateReceiptToInsufficient(message, currentBalance = 0) {
    ...
    icon.innerHTML = `
        <svg width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <path d="M32 20V36M32 44V44.1" stroke="#F59E0B" stroke-width="4" stroke-linecap="round"/>
        </svg>
    `;
    ...
    document.getElementById('receipt-message').innerHTML = `
        ${message}<br>
        <strong>Current balance: ₦${Number(currentBalance).toLocaleString()}</strong><br>
    ...
    actions.style.display = 'flex';
    actions.innerHTML = `
        <button class="receipt-btn primary" id="receipt-fund-wallet">Fund Wallet</button>
        <button class="receipt-btn" id="receipt-done">Close</button>
    `;
...
function updateReceiptToPending(tx = null) {
    ...
    icon.innerHTML = `
        <svg width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <path d="M32 16V32L40 40" stroke="#3B82F6" stroke-width="4" stroke-linecap="round"/>
        </svg>
    `;

    document.getElementById('receipt-status').textContent = 'Pending Delivery';
    document.getElementById('receipt-message').textContent =
        'Your data is being delivered. This may take a few minutes due to network. Money safe - auto refund on fail.';
    ...
    const providerKey =
        data.provider.toLowerCase() === '9mobile' ? 'ninemobile' : data.provider.toLowerCase();
    const svg = svgShapes[providerKey] || '';
    ...
    document.getElementById('receipt-amount').textContent = `₦${Number(displayAmount).toLocaleString(
        'en-NG'
    )}`;
    document.getElementById('receipt-balance').textContent = `₦${Number(
        displayBalance
    ...
    actions.style.display = 'flex';
    actions.innerHTML = `
        <button class="receipt-btn" id="receipt-done">OK</button>
    `;
...
async function pollForFinalStatus(reference) {
    ...
    if (!showedFailed) {
        ...
        if (showedPending) {
            document.getElementById('receipt-message').textContent =
                'Taking longer than expected. Check history later.';
        } else {
            updateReceiptToPending(null);
            document.getElementById('receipt-message').textContent =
                'Delivery taking longer than expected. Check history.';
        }
    }
}
