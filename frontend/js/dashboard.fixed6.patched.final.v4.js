...
(function(){
  ...
  function cacheAuthOptions(rawOpts, converted) {
    try {
      window.__webauthn_rawAuthOptions = rawOpts || null; // JSON-serializable server response
      window.__cachedAuthOptions = converted || null;    // converted for navigator (contains ArrayBuffers)
      window.__cachedAuthOptionsFetchedAt = rawOpts ? Date.now() : 0;
    } catch(e){}
  }
  ...
  function convertOptionsFromServer(publicKey) {
    try {
      ...
      const clone = JSON.parse(JSON.stringify(publicKey)); // keep original strings in raw copy
  ...
  window.getAuthOptionsWithCache = window.getAuthOptionsWithCache || (async function({ credentialId=null, userId=null }={}){
    if (cachedOptionsFresh()) {
      try { return window.__cachedAuthOptions; } catch(e){ return window.__cachedAuthOptions; }
    }
    ...
    const raw = await res.json(); // raw server response with base64url strings
    const converted = convertOptionsFromServer(raw); // converted copy with ArrayBuffers
    cacheAuthOptions(raw, converted);
    return converted;
  });
  window.invalidateAuthOptionsCache = window.invalidateAuthOptionsCache || function(){ cacheAuthOptions(null, null); };
...
// --- Fetch wrapper: intercept direct fetch() calls to /webauthn/auth/options and return raw server JSON when cached ---
// Note: we return a Response that contains the RAW server-format JSON (base64url strings) so code that expects to call res.json()
// will receive data identical to an actual network response. Do NOT stringify objects that include ArrayBuffers.
(function(){
    ...
    window.fetch = async function(input, init){
      try {
        ...
        if (url && url.indexOf('/webauthn/auth/options') !== -1) {
          ...
          // if we have a raw cached server response, return it as a Response with JSON body
          if (window.__webauthn_rawAuthOptions && window.__cachedAuthOptionsFetchedAt && (Date.now() - window.__cachedAuthOptionsFetchedAt) <= (30*1000)) {
            const bodyStr = JSON.stringify(window.__webauthn_rawAuthOptions);
            return new Response(bodyStr, { status: 200, headers: { 'Content-Type': 'application/json' } });
          }
          // otherwise, fall back to the real network call
        }
