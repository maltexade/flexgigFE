...
(function(){
  ...
  // Patch getAuthOptionsWithCache to return converted options and cache them
  try {
    if (typeof window.getAuthOptionsWithCache === 'function' && !window.__webauthn_getAuthPatched) {
      ...
      window.getAuthOptionsWithCache = async function(opts) {
        const raw = await _origGet(opts);
        try {
          const converted = ensurePublicKeyArrayBuffers(raw);
          window.__cachedAuthOptions = converted;
          return converted;
        } catch (err) {
          console.warn('[webauthn] getAuthOptionsWithCache conversion failed', err);
          return raw;
        }
      };
    }
  } catch(e){ console.warn('[webauthn] patch getAuthOptionsWithCache failed', e); }

  // Patch prefetchAuthOptions similarly
  try {
    if (typeof window.prefetchAuthOptions === 'function' && !window.__webauthn_prefetchPatched) {
      ...
      window.prefetchAuthOptions = async function(opts) {
        ...
        try {
          ...
          console.warn('[webauthn] prefetchAuthOptions conversion failed', err);
          return raw;
        }
      };
    }
  } catch(e){ console.warn('[webauthn] patch prefetchAuthOptions failed', e); }

  // Fallback helper to fetch and convert options if nothing else works
  window.getConvertedAuthOptionsFallback = window.getConvertedAuthOptionsFallback || (async function() {
    ...
    try {
      if (typeof window.getAuthOptionsWithCache === 'function') {
        ...
        if (!res.ok) throw new Error('Auth options fetch failed: ' + res.status);
