...
// ------------------------
// verifyBiometrics (drop-in replacement)
// ------------------------
async function verifyBiometrics(uid, context = 'reauth') {
  ...
  // prefer using your withLoader wrapper if present, otherwise run directly
  const runner = (typeof withLoader === 'function') ? withLoader : (fn) => fn();

  // minimal helpers (will use existing ones if present)
  const _base64UrlToBuffer = (typeof base64UrlToBuffer === 'function')
    ? base64UrlToBuffer
    : (s = '') => {
  ...
  const _bufferToBase64Url = (typeof bufferToBase64Url === 'function')
    ? bufferToBase64Url
    : (buffer) => {
        const bytes = new Uint8Array(buffer);
        let str = '';
  ...
  // ensure a robust conversion of many possible incoming types into an ArrayBuffer
  function toArrayBuffer(val) {
    if (val == null) return null;
    if (val instanceof ArrayBuffer) return val;
    if (ArrayBuffer.isView(val)) {
      // copy to normalize offset/length
    ...
    if (val && val.data && Array.isArray(val.data)) {
      // Node-style buffer-like object
      return new Uint8Array(val.data).buffer;
    }
    // otherwise can't coerce
    return null;
  }

  // small promise wrapper to add timeout to navigator.credentials.get
  function getWithTimeout(publicKey, timeoutMs = 60000) {
    return new Promise((resolve, reject) => {
      ...
      // navigator.credentials.get may throw synchronously if options invalid; catch those
      (async () => {
        try {
  ...
  return runner(async () => {
    try {
      ...
      // get cached ready-to-use publicKey options (recommended: call prefetchAuthOptions(uid) earlier)
      let publicKey = window.__cachedAuthPublicKey || null;

      if (!publicKey) {
        // best-effort one-time fetch — note: may break "user gesture" if not invoked in the click handler
        console.warn('[verifyBiometrics] No cached publicKey options; attempting fallback fetch (may lose user gesture).');
        try {
          if (typeof prefetchAuthOptions === 'function') {
            publicKey = await prefetchAuthOptions(uid, context);
          } else {
            // fallback: fetch raw from server and convert locally
            const res = await fetch(`${apiBase}/webauthn/auth/options`, {
              method: 'POST', credentials: 'include',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ userId: uid, context })
            ...
            const opts = await res.json();
            // convert
            if (opts.challenge) opts.challenge = toArrayBuffer(opts.challenge);
            if (Array.isArray(opts.allowCredentials) && opts.allowCredentials.length) {
              opts.allowCredentials = opts.allowCredentials.map(c => ({ ...c, id: toArrayBuffer(c.id) }));
            } else {
          ...
          console.error('[verifyBiometrics] preparing options failed:', e);
          return { success: false, error: 'Could not prepare biometric options' };
        }
      }

      // Ensure allowCredentials ids are ArrayBuffers if present; otherwise remove allowCredentials to use discoverable passkeys
      if (Array.isArray(publicKey.allowCredentials) && publicKey.allowCredentials.length) {
        try {
          publicKey.allowCredentials = publicKey.allowCredentials.map(c => {
            return {
              type: c.type || 'public-key',
              transports: c.transports || c.transport || undefined,
              id: toArrayBuffer(c.id)
            };
          });
          // if any id failed coercion, drop allowCredentials (discoverable path)
          if (publicKey.allowCredentials.some(c => !(c.id instanceof ArrayBuffer))) {
            console.warn('[verifyBiometrics] Some allowCredentials ids could not be coerced -> removing allowCredentials for discoverable flow.');
            delete publicKey.allowCredentials;
          }
        } catch (e) {
          console.warn('[verifyBiometrics] allowCredentials conversion error, dropping to discoverable flow', e);
          delete publicKey.allowCredentials;
        }
      }

      // enforce UV required for strong biometric
      ...
      console.log('[verifyBiometrics] publicKey prepared', {
        rpId: publicKey.rpId, allowCount: publicKey.allowCredentials ? publicKey.allowCredentials.length : 0, challengeLen: publicKey.challenge ? (publicKey.challenge.byteLength || 'unknown') : null
      });

      // Attempt conditional mediation first (fast/silent if supported)
      let assertion = null;
      ...
      try {
        if (typeof PublicKeyCredential !== 'undefined' && typeof PublicKeyCredential.isConditionalMediationAvailable === 'function') {
          conditionalAvailable = await (async () => {
            try { return await PublicKeyCredential.isConditionalMediationAvailable(); } catch (e) { return false; }
          })();
        }
      } catch (e) {
      ...
      if (conditionalAvailable) {
        try {
          console.log('[verifyBiometrics] Attempting conditional mediation (short timeout)');
          // copy publicKey to avoid mutation issues
          const condOpts = JSON.parse(JSON.stringify(publicKey));
          // browsers accept mediation: 'conditional' on top-level of credential request options,
          // but the shape is { publicKey, mediation: 'conditional' } when passed to navigator.credentials.get
          // We'll pass directly via navigator.credentials.get({ publicKey: condOpts, mediation: 'conditional' })
          // but to be maximally interoperable we set condOpts.mediation as separate below:
          // Use short timeout for conditional attempt
          assertion = await (async () => {
            // some browsers do not like extra unknown props, so pass as two-arg-like object
            // We'll use race timeout wrapper
            try {
              // navigator.credentials.get may ignore unknown properties; so pass publicKey only.
              // conditional mediation is requested by passing mediation: 'conditional' as second property - but not all browsers accept it.
              // The safest cross-browser code is to call navigator.credentials.get({ publicKey: condOpts, mediation: 'conditional' })
              return await getWithTimeout({ ...condOpts, mediation: 'conditional' }, 7000);
            } catch (err) {
              // attempt without adding mediation on the object (some engines support the static API)
              try {
                return await getWithTimeout(condOpts, 7000);
              } catch (err2) {
                throw err;
              }
            }
          })();
          console.log('[verifyBiometrics] conditional result:', !!assertion);
        } catch (condErr) {
          console.warn('[verifyBiometrics] conditional failed/timeout:', condErr && (condErr.name || condErr.message) || condErr);
          assertion = null;
        }
      } else {
        console.log('[verifyBiometrics] conditional mediation not available');
      }

      // If conditional didn't return an assertion, fallback to interactive prompt (user prompt)
      if (!assertion) {
        console.log('[verifyBiometrics] Falling back to interactive prompt (user prompt)');
        const maxRetries = 3;
        let attempt = 0;
        let lastError = null;
        while (attempt < maxRetries && !assertion) {
          attempt++;
          try {
            assertion = await getWithTimeout(publicKey, publicKey.timeout || 60000);
            console.log('[verifyBiometrics] interactive get() succeeded on attempt', attempt);
            break;
          } catch (err) {
            lastError = err;
            const name = err && err.name ? err.name : String(err);
            console.warn(`[verifyBiometrics] interactive attempt ${attempt} failed:`, name, err && err.message ? err.message : err);

            // If a request is already pending, wait and retry
            if (name.includes('A request is already pending') || name.includes('A request is already in progress')) {
              await new Promise(r => setTimeout(r, 500 + attempt * 200));
              continue;
            }
            // If user cancelled or not allowed, stop retrying
            if (name === 'NotAllowedError' || name === 'AbortError') {
              break;
            }
            // Otherwise allow one more retry
            await new Promise(r => setTimeout(r, 200));
          }
        }
        if (!assertion && lastError) {
          // bubble the last error name for caller handling
          const errName = (lastError && (lastError.name || lastError.message)) || 'interactive_failed';
          console.error('[verifyBiometrics] interactive attempts exhausted:', errName);
      ...
      if (!assertion) {
        console.warn('[verifyBiometrics] No assertion returned');
        return { success: false, error: 'No assertion returned' };
      }

      // Build the credential payload to send to server
      const cred = {
        id: assertion.id,
      ...
      console.log('[verifyBiometrics] assertion processed ->', { id: cred.id, rawIdLen: cred.rawId.length, userHandlePresent: !!cred.response.userHandle });
      ...
      try {
        const verifyRes = await fetch(`${apiBase}/webauthn/auth/verify`, {
          method: 'POST', credentials: 'include',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ userId: uid, credential: cred, context })
        });

        if (!verifyRes.ok) {
          const txt = await verifyRes.text().catch(() => String(verifyRes.status));
          console.error('[verifyBiometrics] server verify failed', verifyRes.status, txt);
          // If server reports mismatch, consider clearing stored credentialId to avoid repeated fails
          try {
            const stored = localStorage.getItem('credentialId');
            if (stored) {
              console.warn('[verifyBiometrics] Clearing stored credentialId due to server verify failure');
              localStorage.removeItem('credentialId'); sessionStorage.removeItem('credentialId');
              document.cookie = `fg_credentialId=;expires=${new Date(0).toUTCString()};path=/`;
            }
          } catch (e) { /* ignore */ }
          return { success: false, error: `Server verify failed (${verifyRes.status})` };
        }
        ...
        // Persist credentialId if server canonicalized it
        try {
          const persisted = (json && (json.credentialId || json.id)) ? (json.credentialId || json.id) : cred.rawId;
          if (typeof persistCredentialId === 'function') persistCredentialId(persisted);
          else localStorage.setItem('credentialId', persisted);
        } catch (e) {
          console.warn('[verifyBiometrics] persistCredentialId failed', e);
        }
        ...
        return { success: true, result: json || { credential: cred } };
      } catch (netErr) {
        console.error('[verifyBiometrics] network error during verify', netErr);
        return { success: false, error: 'Network error during verify' };
      }
    } catch (err) {
      console.error('[verifyBiometrics] ERROR', err);
      // treat user cancel/timeouts specially
