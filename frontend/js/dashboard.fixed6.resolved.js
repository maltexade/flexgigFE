...
/* -----------------------------
  Reauth Conflict & UX Fixes (appended)
  - 1) Prevent phone-number inputs from being targeted as reauth PINs
  - 2) Sync authenticator toggle with actual authenticator presence on modal open / load
  - 3) Make biometric prompt appear faster by attempting a pointerdown-started verification and show loader immediately while awaiting server response
----------------------------- */
(function(){
  // Utility selectors - change if your modal/input ids differ
  const REAUTH_MODAL_SELECTORS = ['#reauthModal', '#reauth-modal', '.reauth-modal', '[data-reauth-modal]'];
  const AUTH_TOGGLE_SELECTORS = ['#authToggle', '[data-auth-toggle]', '.webauthn-toggle'];
  const BIO_BUTTON_SELECTORS = ['#pinBiometricBtn', '#bioBtn', '.biometric-button', '[data-bio-button]'];

  function findReauthModal() {
    for (const s of REAUTH_MODAL_SELECTORS) {
      const el = document.querySelector(s);
      if (el) return el;
    }
    // fallback: try element that appears when reauthModalOpen flag is set
    return document.querySelector('[data-reauth]') || null;
  }

  // 1) Ensure we only touch inputs that belong to the reauth modal
  function getReauthInputsNamespaced() {
    try {
      const modal = findReauthModal();
      if (!modal) {
        // fallback to previous behaviour: try to find inputs with class '.pin-input' but exclude tel/phone inputs
        return Array.from(document.querySelectorAll('input.pin-input, input.reauth-pin')).filter(i => !/tel|phone|mobile/i.test(i.name || i.id || i.className || ''));
      }
      // within modal, pick inputs that look like PIN boxes but exclude phone inputs
      ...
      return candidates.filter(i => {
        ...
        if (t === 'tel' || /phone|tel|mobile/.test(id) || /phone|tel|mobile/.test(name) || /phone|tel|mobile/.test(cls)) return false;
        // Heuristic: accept short single-char inputs or ones marked reauth/pin
        if (cls.includes('pin') || cls.includes('reauth') || id.includes('pin') || name.includes('pin')) return true;
        // accept if maxLength <= 2 (likely PIN box)
        if (i.maxLength && Number(i.maxLength) <= 2) return true;
        // else reject
        return false;
      });
    } catch(e){ console.warn('getReauthInputsNamespaced error', e); return []; }
  }

  // Override existing helper if present
  window.getReauthInputs = window.getReauthInputs || getReauthInputsNamespaced;

  // 2) Sync authenticator toggle with server/available authenticators
  async function syncAuthenticatorToggleOnOpen() {
    try {
      const modal = findReauthModal();
      if (!modal) return;
      // Attempt to obtain authenticator list via multiple fallbacks
      let hasAuth = null;
      // 2a: If app exposes a function to list authenticators
      if (typeof window.listAuthenticators === 'function') {
        try { const list = await window.listAuthenticators(); hasAuth = Array.isArray(list) && list.length > 0; } catch(e) { console.warn('listAuthenticators failed', e); }
      }
      // 2b: If app exposes getAuthStatus
      if (hasAuth === null && typeof window.getAuthenticatorStatus === 'function') {
        try { hasAuth = Boolean(await window.getAuthenticatorStatus()); } catch(e) {}
      }
      // 2c: Network fallback - try an endpoint that many apps expose. This is best-effort and won't crash if CORS blocked.
      if (hasAuth === null) {
        try {
          const resp = await fetch('/webauthn/auth/list', { credentials: 'include' });
          if (resp && resp.ok) {
            const json = await resp.json();
            if (Array.isArray(json)) hasAuth = json.length > 0;
            else if (json && json.count !== undefined) hasAuth = (json.count > 0);
          }
        } catch(e){ /* ignore */ }
      }
      // Default to false if still null (safe fallback)
      if (hasAuth === null) hasAuth = false;
      // Update toggle UI to reflect actual state: if no authenticators, force toggle off
      for (const sel of AUTH_TOGGLE_SELECTORS) {
        ...
        try {
          if (el.type === 'checkbox' || el.getAttribute('role') === 'switch') {
            ...
            // toggle represented by class
            if (hasAuth) el.classList.add('active'); else el.classList.remove('active');
          }
        } catch(e){ console.warn('update toggle failed', e); }
      }
      // Also ensure internal settings reflect this
      if (!hasAuth && typeof window.setAuthToggleState === 'function') {
        try { window.setAuthToggleState(false); } catch(e){}
      }
    } catch(e){ console.warn('syncAuthenticatorToggleOnOpen error', e); }
  }

  // Call sync when modal opens and on load if lock present
  (function watchModalOpen(){
    // If app sets reauthModalOpen flag, watch it
    try {
      const origShow = window.showReauthModal;
      if (typeof origShow === 'function' && !origShow.__syncWrapped) {
        window.showReauthModal = async function(ctx){
          const res = await origShow(ctx);
          setTimeout(syncAuthenticatorToggleOnOpen, 30);
          return res;
        };
        window.showReauthModal.__syncWrapped = true;
      }
    } catch(e){}
    // Also attempt on load if reauth lock present
    try { if (localStorage.getItem('fg_reauth_lock_v1')) setTimeout(syncAuthenticatorToggleOnOpen, 200); } catch(e) {}
    // Mutation observer to detect modal insertion into DOM if app directly injects elements
    const mo = new MutationObserver((mut)=>{
      for (const m of mut) {
        if (m.addedNodes && m.addedNodes.length) {
          for (const n of m.addedNodes) {
            if (n && n.querySelector && n.querySelector('.reauth') ) {
              setTimeout(syncAuthenticatorToggleOnOpen, 50);
              return;
            }
          }
        }
  ...
  // 3) Speed up biometric prompt: attempt a pointerdown-started verification and show loader immediately
  (function fastBiometricStart(){
    try {
      const seen = new WeakSet();
      const attemptStart = async function(btn){
        if (!btn || seen.has(btn)) return;
        seen.add(btn);
        // On pointerdown we call prefetch and then start the verification flow immediately (user gesture preserved)
        btn.addEventListener('pointerdown', async function(ev){
          try {
            // prefetch to warm challenges
            window.prefetchAuthOptions && window.prefetchAuthOptions();
            // show loader instantly to indicate process started (if defined)
            try { showLoader && showLoader(); } catch(e){}
            // Attempt to call robust verify directly (navigator will show prompt)
            ...
            // Mark that verification is in-flight to avoid duplicate triggers
            if (btn.__fg_verifying) return;
            btn.__fg_verifying = true;
            const res = await verifyBiometricsRobust(uid, 'reauth');
            // hide loader if verifyBiometricsRobust didn't already do so
            try { hideLoader && hideLoader(); } catch(e){}
            // if failure, reveal PIN view fallback after small delay
            if (!res || !res.success) {
              setTimeout(()=>{ try { if (typeof switchViews === 'function') switchViews(false); } catch(e){} }, 200);
            }
            btn.__fg_verifying = false;
          } catch(err){
            console.warn('fast biometric pointerdown handler error', err);
            try { hideLoader && hideLoader(); } catch(e) {}
            btn.__fg_verifying = false;
          }
        }, { passive: false });
      };
      // Bind to known selectors
      const bindAll = ()=>{
        const els = BIO_BUTTON_SELECTORS.flatMap(s => Array.from(document.querySelectorAll(s)));
        const uniq = Array.from(new Set(els));
        uniq.forEach(el => attemptStart(el));
      };
      bindAll();
      // Observe DOM changes and bind again
      const mo = new MutationObserver(bindAll);
      mo.observe(document.body, { childList:true, subtree:true });
    } catch(e){ console.warn('fastBiometricStart init failed', e); }
  })();

  // Small safeCall helper used by extra logic (non-destructive)
  function safeCall(fn, ...args) {
  ...
  console.log('[reauth-patch] Conflict fixes and fast-biometric-start appended');
})();
