...
(() => {
  ...
  try {
    ...
    try {
      ...
      // trigger UI sync and prefetch so biometric button becomes ready without reload
      try { if (typeof window.reconcileBiometricState === 'function') window.reconcileBiometricState(); } catch(e){}
      try { if (typeof window.prefetchAuthOptions === 'function') window.prefetchAuthOptions(); } catch(e){}
    } catch(e){}
    return { ok: true };
  } catch (err) {
...
if (!opts) {
  ...
  return;
    ...
    setTimeout(() => {
      ...
      bioBtn.addEventListener('click', async () => {
        ...
        // Show PIN/modal immediately (PIN view already visible, but ensure modal is open)
        try {
  ...
  // --------- KEY CHANGE: FORCE PIN VIEW (no view switching) ----------
  try {
    // Always show PIN view and hide biometric view
    if (biometricView) biometricView.style.display = 'none';
    if (pinView) pinView.style.display = 'block';

    // Hide any UI that allows switching away (keep elements if other code references them)
    try { if (switchToBiometric) switchToBiometric.style.display = 'none'; } catch (e) {}
    try { if (switchToPin) switchToPin.style.display = 'none'; } catch (e) {}

    console.log('Views set - forced PIN view (biometric hidden)');
  } catch (e) {
    console.error('Error forcing PIN view:', e);
  }

  // Resume lockout countdowns if provided
  try {
  ...
  // Bind PIN inputs (same as before)
  try {
  ...
  // Biometric verify button: keep as manual fallback (unchanged)
  try {
    ...
    if (verifyBiometricBtn && !verifyBiometricBtn.__bound) {
      verifyBiometricBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          // fallback to PIN view (already visible)
        } catch (err) {
  ...
  // Remove/disable view switches (no user toggling views)
  try {
    console.log('Disabling view switches (modal forced to PIN view)');
    try { if (switchToPin) switchToPin.style.display = 'none'; } catch(e){}
    try { if (switchToBiometric) switchToBiometric.style.display = 'none'; } catch(e){}
  } catch (e) {
    console.error('Error disabling view switches:', e);
  }

  // Logout & forget links (unchanged)
  try {
  ...
  // Modal visibility and focus handling
  try {
    ...
    if (reauthModal) {
      ...
      // Focus the first PIN input for usability
...
async function verifyBiometrics(uid, context) {
  ...
  return withLoader(async function(){
    try {
      ...
      // Prefer a fresh challenge for actual authentication (avoid single-use challenge reuse)
      let publicKey = null;
      try {
        if (typeof window.getFreshAuthOptions === 'function') {
          publicKey = await window.getFreshAuthOptions({});
        } else if (typeof window.getAuthOptionsWithCache === 'function') {
          // attempt to invalidate then fetch fresh via helper if available
          if (typeof window.invalidateAuthOptionsCache === 'function') try { window.invalidateAuthOptionsCache(); } catch(e){}
          publicKey = await window.getAuthOptionsWithCache({});
        } else {
          // fallback: use cached if present and recent
          publicKey = window.__cachedAuthOptions || null;
          ...
          if (!publicKey) {
            ...
            try {
              const optRes = await (window.__origFetch || fetch)((window.__SEC_API_BASE || API_BASE) + '/webauthn/auth/options', {
                method: 'POST',
                credentials: 'include',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ userId: userId, credentialId: localStorage.getItem('credentialId') || null })
              });
              const txt = await optRes.text();
              publicKey = txt ? JSON.parse(txt) : null;
            } catch(e){
              publicKey = null;
            }
          }
        ...
        console.warn('[verifyBiometrics] fresh options fetch failed, falling back to cache', e);
        publicKey = window.__cachedAuthOptions || null;
      }

      if (!publicKey) {
        throw new Error('Could not obtain WebAuthn options for authentication');
      }
