    ...
    (function(){
      ...
      try {
        ...
        if (url && url.indexOf('/webauthn/auth/options') !== -1) {
          ...
          // Serve cached converted options if present, otherwise forward to original fetch (no recursion)
          if (window.__cachedAuthOptions) {
            return new Response(JSON.stringify(window.__cachedAuthOptions), { status: 200, headers: { 'Content-Type': 'application/json' } });
          }
