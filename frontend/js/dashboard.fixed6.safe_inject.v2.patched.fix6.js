...
(function () {
  ...
  if (typeof window.fetch === 'function') {
      ...
      window.fetch = async function (input, init) {
        try {
          ...
          if (url && url.indexOf('/webauthn/auth/options') !== -1) {
            ...
            // Serve cached converted options if present, otherwise forward to original fetch
            if (window.__cachedAuthOptions) {
              return new Response(
                JSON.stringify(window.__cachedAuthOptions),
                { status: 200, headers: { 'Content-Type': 'application/json' } }
              );
            }

            // Otherwise proceed normally (no recursion)
    ...
    })(); // <-- properly close inner IIFE
  }
})(); // <-- properly close outer IIFE
