...
// ADD THIS TO YOUR dashboard.js (after onDashboardLoad function)
 ...
 * @param {string} orderData.phoneNumber - Recipient phone number
 * @param {string} orderData.provider - Network provider (MTN, Airtel, etc)
 * @param {string} orderData.planName - Data plan name (e.g., "1GB - 30 Days")
 * @param {string} orderData.price - Price in Naira
 * @param {string} orderData.planId - Plan identifier for API
 * @param {string} orderData.networkCode - Network code for API
 */
function openCheckoutModal(orderData) {
  try {
    console.log('[Checkout] Opening with data:', orderData);
    ...
    if (!phoneEl || !priceEl || !dataEl || !providerEl || !payBtn) {
      console.error('[Checkout] Required elements not found');
      if (typeof notify === 'function') {
        notify('Checkout unavailable. Please refresh the page.', 'error');
      }
      return;
    }
    
    // Set values
    phoneEl.textContent = orderData.phoneNumber || localStorage.getItem('phoneNumber') || '080XXXXXXXX';
    priceEl.textContent = `₦${orderData.price || '0'}`;
    dataEl.textContent = orderData.planName || 'N/A';
    providerEl.textContent = orderData.provider || 'N/A';
    
    // Store order data for payment
    window.__checkoutData = {
      phoneNumber: phoneEl.textContent,
      provider: orderData.provider,
      planName: orderData.planName,
      price: orderData.price,
      planId: orderData.planId,
      networkCode: orderData.networkCode
    };
    ...
    if (window.ModalManager && typeof window.ModalManager.openModal === 'function') {
      ...
      console.error('[Checkout] ModalManager not available');
      if (typeof notify === 'function') {
        notify('Modal system unavailable', 'error');
      }
    }
  } catch (err) {
    console.error('[Checkout] Error opening modal:', err);
    if (typeof notify === 'function') {
      notify('Failed to open checkout', 'error');
    }
  }
}
...
async function processCheckoutPayment() {
  ...
  const orderData = window.__checkoutData;
  
  if (!orderData) {
    console.error('[Checkout] No order data found');
    if (typeof notify === 'function') {
      notify('Invalid order data', 'error');
    }
    return;
  }
  
  try {
    console.log('[Checkout] Processing payment:', orderData);
    ...
    const endpoint = `${apiBase}/api/purchase/data`;
    ...
    console.log('[Checkout] API request:', endpoint, requestBody);
    ...
    try {
      ...
      console.error('[Checkout] Failed to parse response:', e);
    ...
    console.log('[Checkout] Purchase successful:', result);
    ...
    if (typeof notify === 'function') {
      notify(result?.message || 'Purchase successful!', 'success');
    }
    
    // Refresh user data to update balance
    if (typeof getSession === 'function') {
      setTimeout(() => {
        getSession().catch(e => console.warn('[Checkout] Failed to refresh session:', e));
      }, 500);
    }
    
    // Clear checkout data
    delete window.__checkoutData;
    
    // Optional: Trigger transaction history refresh
    if (typeof refreshTransactionHistory === 'function') {
      setTimeout(refreshTransactionHistory, 1000);
    }
...
// Bind pay button click handler
document.addEventListener('DOMContentLoaded', () => {
  const payBtn = document.getElementById('payBtn');
  if (payBtn) {
    payBtn.addEventListener('click', processCheckoutPayment);
    console.log('[Checkout] Pay button handler attached');
  }
});

// ==================== EXAMPLE USAGE ====================

// Example 1: Open checkout from a plan selection
// Add this to your plan cards' buy buttons
/*
document.querySelectorAll('.buy-plan-btn').forEach(btn => {
  btn.addEventListener('click', function() {
    const orderData = {
      phoneNumber: localStorage.getItem('phoneNumber') || '080XXXXXXXX',
      provider: this.dataset.provider, // e.g., 'MTN'
      planName: this.dataset.planName, // e.g., '1GB - 30 Days'
      price: this.dataset.price, // e.g., '500'
      planId: this.dataset.planId, // e.g., 'mtn_1gb_30d'
      networkCode: this.dataset.networkCode // e.g., '1' for MTN
    };
    
    openCheckoutModal(orderData);
  });
});
*/

// Example 2: Open from allPlansModal (after user selects a plan)
/*
function handlePlanSelection(plan) {
  // Close all plans modal first
  if (window.ModalManager) {
    window.ModalManager.closeModal('allPlansModal');
  }
  
  // Wait for close animation, then open checkout
  setTimeout(() => {
    openCheckoutModal({
...
/**
 * Get user wallet balance (cached or fetch)
 */
  ...
  try {
    ...
    if (userData) {
      ...
      if (parsed.walletBalance !== undefined) {
        return parseFloat(parsed.walletBalance) || 0;
      }
    }
    ...
    console.warn('[Checkout] Failed to get wallet balance:', e);
  }
  return 0;
}
...
window.getUserWalletBalance = getUserWalletBalance;
...
function initializeSmartAccountPinButton() {
    try {
        ...
        function focusFirstInput(mode) {
            // Broader selector: Any active/open PIN modal
            const modal = document.querySelector('.modal.active, .pin-modal:not(.hidden), #pinModal:not(.hidden), #securityPinModal:not(.hidden)');
            if (modal) {
                const firstInput = modal.querySelector('input[type="password"], input[autofocus], .pin-input');
                if (firstInput) {
                    firstInput.focus();
  ...
  if (step === 'confirm') {
    ...
    return withLoader(async () => {
      try {
        ...
        if (accountPinStatus) accountPinStatus.textContent = 'PIN set. You can change your PIN here';
