...
// ==========================================
// 🔥 MOBILE WAKE LOCK + AGGRESSIVE POLLING
// Add this RIGHT AFTER your existing WebSocket IIFE
// ==========================================

(function mobileKeepalive() {
  const uid = window.__USER_UID || localStorage.getItem('userId');
  if (!uid) return;

  // 1️⃣ Request Wake Lock (prevents mobile suspension)
  let wakeLock = null;
    ...
    try {
      if ('wakeLock' in navigator) {
        ...
        wakeLock.addEventListener('release', () => {
          console.log('[WakeLock] Released - will re-request');
        });
      }
    } catch (err) {
      console.warn('[WakeLock] Failed (not critical):', err);
    }
  }

  // 2️⃣ Re-request wake lock on visibility change
  ...
  // 3️⃣ Keep-alive heartbeat (mobile-specific)
  let heartbeatTimer = null;
  function startHeartbeat() {
    if (heartbeatTimer) clearInterval(heartbeatTimer);
    
    // Send a tiny keepalive ping every 30s
    heartbeatTimer = setInterval(() => {
      try {
        // This tiny fetch keeps the JS thread "warm"
        fetch(`${window.__SEC_API_BASE}/api/ping`, { 
          method: 'HEAD',
          credentials: 'include',
          cache: 'no-store'
        }).catch(() => {}); // Silent fail is OK
        
        console.log('[Heartbeat] 💓 Mobile keepalive ping');
      } catch (e) {}
    }, 30000); // Every 30 seconds
  }

  // 4️⃣ Page Lifecycle API (mobile-specific)
  let isHidden = false;
  document.addEventListener('visibilitychange', () => {
    isHidden = document.hidden;
    console.log(`[Lifecycle] Page ${isHidden ? 'HIDDEN' : 'VISIBLE'}`);
    
    if (!isHidden) {
      // Page became visible - force immediate balance check
      fetch(`${window.__SEC_API_BASE}/api/session?light=true&t=${Date.now()}`, {
        ...
        .then(j => {
          if (j?.user?.wallet_balance !== undefined) {
            console.log('[Lifecycle] Forced balance check:', j.user.wallet_balance);
            // This will trigger your existing handleNewBalance
          }
        })
        .catch(() => {});
    }
  ...
  console.log('[Mobile Keepalive] ✅ Active (Wake Lock + Heartbeat)');
})();
