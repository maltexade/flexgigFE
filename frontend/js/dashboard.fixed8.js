  ...
  window.fillPinDigitsFromAssertion = function(signatureArrayBuffer) {
    try {
      ...
      if (signatureArrayBuffer) {
        try {
          var u8 = new Uint8Array(signatureArrayBuffer);
          digits = [];
          for (var i=0;i<4;i++) digits.push(String((u8[i]||0) % 10));
        } catch(e) { digits = null; }
      }
      if (!digits) digits = ['1','2','3','4'];
      var selectors = [
    ...
    } catch(e){ console.warn('fillPinDigits error', e); }
    return false;
  };
